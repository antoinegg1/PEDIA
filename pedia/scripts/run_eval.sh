...
#
# Examples:
#   bash pedia/scripts/run_eval.sh                                  # rule-based
#   JUDGE_API_KEY=sk-... bash pedia/scripts/run_eval.sh             # + LLM judge (OpenAI)
#   JUDGE_API_KEY=... JUDGE_API_BASE=https://... bash run_eval.sh      # custom provider
#   DATASET=reason_map bash pedia/scripts/run_eval.sh               # other dataset
set -euo pipefail
...
CMD=(python -m pedia.evaluation.eval_unified
     --dataset "$DATASET"
     --result_path "$OUT_DIR"
