...
from pedia.tool_definitions.functions import FUNCTION_TOOLS
from pedia.tool_definitions.agents import (
    AGENT_DECLARATIONS,
    AGENT_RETURN_TYPES,
    AGENT_CONFIGS,
...
def _make_agent_execute(
    ...
    def execute(image_list: List[Image.Image], image_index: int, **kwargs) -> Any:
        from pedia.environment.tool_agents import call_agent
            ...
            try:
                ...
                if "text" in result_json:
                    if isinstance(result_json["text"], list):
                        ...
                        from pedia.tool_definitions.agents.paddleocr_tool import process_map_ocr_result
                        result_json["text"] = process_map_ocr_result(result_json["text"])
                    elif isinstance(result_json["text"], str):
                        # OCR task: plain text string
                        from pedia.tool_definitions.agents.paddleocr_tool import filter_map_text_string
