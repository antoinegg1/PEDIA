...
Usage:
    python -m pedia.evaluation.eval_unified \
...
from pedia.utils.io_utils import iter_meta_info_files, load_records
from pedia.utils.stats import get_output_tokens_total, get_input_tokens_total
...
def main():
    ...
    parser.add_argument(
        ...
        help="Dataset id (auto-resolves eval template via "
             "pedia.evaluation.eval_datasets.DATASET_REGISTRY). If omitted, --dataset_name required.",
    )
    ...
    with tqdm(total=len(record_items), desc="evaluating") as pbar:
        ...
        if args.use_judge and args.dataset_name != "map_trace":
            ...
            if api_key:
                from pedia.evaluation.openai_as_judge import OpenAIJudge
    ...
    if args.dataset_name in ("3dsrbench", "babyvision", "visworld_eval", "omnispatial"):
